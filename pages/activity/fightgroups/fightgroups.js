...
// var openid;
// var unionid;
var shareTitle = "送你一份初级会计职称资料包，点击立即领取";
// var shareImg = "../../../image/fightgroups/share_cjkjzc.png";
var shareImg = "";
var groupid = "";
var templateid = "";
  ...
  countDownHandler: function () {
    ...
    if (!interval) {
      interval = setInterval(() => {
        ...
        var addtimestamp = Date.parse(this.strToDate(this.data.templateList[0].addtime.replace(/-/g, '/')));
        ...
        if (addtimestamp - timestamp <= 0) {
          this.setData({ countDownTimeTitle: "拼团已结束" });
          this.setData({ fightGroupsIsOverState: "1" });
          this.stopcountDownHandler();
          // wx.showModal({
          //   title: '温馨提示',
          //   content: '该拼团已过期失效，是否重新发起',
          //   confirmText: "确定",
          //   cancelText: '取消',
          //   success: function (res) {
          //     if (res.confirm) {
  ...
  */
  ...
  getUserInfoNext: function () {
    ...
    swan.getUserInfo({
      success: function (res) {
        ...
        common.showModal({
          title: '警告',
          content: '您点击了拒绝授权，将无法正常获取您的微信头像、昵称。请10分钟后再次点击授权，或者删除小程序重新进入。',
          showCancel: false,
          success: function (res) {
            if (res.confirm) {
              //console.log('用户点击确定')
  ...
  checkOpenPower: function () {
    ...
    if (groupid == undefined || templateid == undefined || fightgroupsover == 1) {
      ...
      this.setGroups();
      // wx.setStorageSync('fightgroupsover', 0)
    } else {
