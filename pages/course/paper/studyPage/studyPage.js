...
Page({
  ...
  data: {
    ...
    // 视频播放器参数
    videoUrl: '', // url
    hiddenVideo: true, // 默认隐藏
    videoAutoPlay: false, // 默认不自动播放
    videoContext: '', // video上下文

    doubleTimes: 0, // checkboxChange执行两次，answerIndex会变的问题
    answerIndex: '',
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  checkboxChange: function (e) {
    ...
    this.setData({doubleTimes: this.data.doubleTimes + 1});
    let doubleTimes = this.data.doubleTimes;
    var answerIndex;
    if (doubleTimes == 1) {
      answerIndex = e.currentTarget.dataset.hi;
      this.setData({answerIndex: answerIndex});
    } else {
      answerIndex = this.data.answerIndex;
      this.setData({doubleTimes: 0});
    }
