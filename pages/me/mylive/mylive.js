...
Page({
  ...
  getlivelistByCategoryid: function (index) {
    ...
    api.getlivelistByCategoryid({
      methods: 'POST',
      data: {
        // categoryid: this.data.courselist[index].categoryid,
        uid: uid
      },
      success: res => {
        swan.hideToast();
  ...
  recentCellClick: function (event) {
    ...
    if (this.data.recent_list[index].state == 2 && nowTime == this.data.recent_list[index].liveday) {
      ...
      this.checkcourse(courseid, videotype, channelnumber, chatroomid, index, "recent");
    }
  },
  ...
  */
  ...
  handleTap: function (event) {
    var index = event.currentTarget.dataset.index;
    // if (index - 1 == wx.getStorageSync('myLiveNavIndex')) {
    //   return;
    // }
